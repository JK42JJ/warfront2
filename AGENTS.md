...
1-b. **모든 문항은 실전형 지문(statement) 필수.** 화면은 좌 지문 / 우 코드
   2컬럼(실전 플랫폼과 동일 배치). ko=프로그래머스체, en=HackerRank체
   (statement_lang, 1순위 앵커 플랫폼 언어). 지문의 제한 수치는 perf.gen과,
   예제는 tests와 정합해야 한다. 절차: .claude/skills/wf-add-problem/SKILL.md
...
- 도트 글리프(▰▶▱ 등)를 한글·숫자와 **붙여 쓰면 실터미널에서 겹쳐 보인다**
  (폰트는 광폭으로 그리는데 셀 계산은 1칸). 반드시 뒤에 공백 (2026-07-21 실사용 사고)

...
연습할 때는 "익은 토마토가 하루마다 옆 토마토를 익게 한다, 전부 익는 데
며칠 걸리나" 같은 문제로 탐색을 배웁니다. 그런데 시험에는 "감염된 서버가
매 시간 인접 서버를 감염시킨다"로 나옵니다. 이야기만 다르지 같은
문제인데, 못 알아봅니다. 문제집은 챕터 제목이 유형을 미리 알려주기
...
사라집니다. 여기서는 단계 해금이 진도 그 자체입니다. 대시보드가 패턴마다
...
**같은 패턴을 네 번, 조금씩 더 어렵게.** 운전을 배울 때 옆에서 보고,
연수 코스를 돌고, 혼자 몰아보는 순서와 같습니다. 한 패턴(예: 탐색 코드)을
이렇게 네 단계로 반복합니다.

```
① 보고 치기      코드 전체가 화면에 보입니다. 그대로 따라 치면서
                 손과 눈에 익힙니다.
                 │ from collections import deque      ← 전부 보임
                 │ q = deque([start])

② 빈칸 채우기    핵심 부분만 가려집니다. 그 자리를 떠올려서 채웁니다.
                 │ from collections import deque      ← 주어짐
                 │ ·································   ← 여기만 직접

③ 안 보고 재현   전부 가려집니다. 기억만으로 처음부터 끝까지 씁니다.
                 │ ·································
                 │ ·································

④ 스스로 구현    이번엔 코드 없이 문제만 나옵니다. 빈 에디터에서
                 직접 짜고 채점을 받습니다. 답과 코드가 달라도
                 동작이 맞으면 통과입니다.
```

한 단계에서 정확도 95%를 넘기면 다음 단계가 열립니다. 패턴은 한국 코테
출제 빈도 순서로 15종 — 구현, 해시, 그리디, 스택, 힙, 백트래킹, BFS, DFS,
문자열, DP, 이분탐색, 투포인터, 다익스트라.

![따라치기](docs/img/kata.svg)

치는 동안 글자마다 바로 판정됩니다. 맞으면 흰색, 틀리면 빨간색이 되고
틀린 글자에서는 앞으로 나아가지 않습니다. 막히면 F1이 "어떤 순서로
만드는지" 힌트를, F2가 알고리즘이 움직이는 모습을 그림으로 보여줍니다.

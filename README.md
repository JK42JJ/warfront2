...
두 번째가 이 도구의 핵심입니다. 연습 때는 "익은 토마토가 하루마다 옆
토마토를 익게 한다, 전부 익는 데 며칠?"로 배우는데, 시험에는 "감염된
서버가 매 시간 인접 서버를 감염시킨다"로 나옵니다. 이야기만 다르지 같은
문제인데 못 알아봅니다. 문제집은 챕터 제목이 유형을 미리 알려주므로,
시험의 절반인 "알아보기"를 연습할 기회가 없었던 겁니다.

## 훈련 흐름

```mermaid
flowchart LR
    T["🧠 접근 선언<br/>(생각 먼저)"] --> A["① 보고 치기"]
    A -->|"95%"| B["② 빈칸 채우기"]
    B -->|"95%"| C["③ 안 보고 재현"]
    C -->|"95%"| D["④ 스스로 구현<br/>테스트 채점"]
    D --> R["기록 저장<br/>GitHub 백업"]
    I["🔍 인식 드릴<br/>(유형 알아보기)"] -.매일 5분.-> T
```

모든 문제는 접근을 한 줄 선언해야 에디터가 열립니다. 머리로든 연필로든
구조를 먼저 그리고 코드를 치는 습관을 만들기 위해서입니다.
...
- ④는 히든 테스트케이스와 대형 입력 시간제한으로 채점합니다.
  모범답안과 코드가 달라도 동작이 맞으면 통과, 맞아도 느리면 탈락 —
  실제 채점 환경 그대로입니다.
...
이런 경계를 틀릴 때마다 그 자리에서 배우고, 인식률 90%가 목표선입니다.
...
macOS는 아무 터미널이나, Windows는 Windows Terminal을 권장합니다
(`wf`가 안 잡히면 `py -m wf.cli`).
...
대시보드에서 `t` — 7일 압축 과정. 출제 빈도 최상위 유형만 하루 다섯 개씩
(새 패턴 + 전날 복습), 마지막 날은 전체 리허설. 그날 할 것에 ⚡와 🔁가 붙습니다.
...
빈 private 저장소 하나면 됩니다. 세션이 끝날 때마다 그날의 기록이 자동으로
올라가고, 다른 컴퓨터에서 같은 명령을 치면 진도가 복원됩니다.
